...
class DatePickerHeader extends React.Component {
  ...
  render() {
    return <div className="text-center">
      <div className="text-muted float-left datepicker-previous-wrapper" onClick={this.handleClickPrevious.bind(this)} style={{cursor: 'pointer'}}>
      ...
      <div className="text-muted float-right datepicker-next-wrapper" onClick={this.handleClickNext.bind(this)} style={{cursor: 'pointer'}}>
...
export default class DatePicker extends React.Component {
  ...
  renderClearButton() {
      if(this.props.customControl || !this.props.showClearButton)
        return null;
      else {
        const onClick = this.props.disabled ? null : this.clear.bind(this);
        const appendStyle = {cursor: (this.state.inputValue && !this.props.disabled) ? 'pointer' : 'not-allowed'};
        const textStyle = {opacity: (this.state.inputValue && !this.props.disabled) ? 1 : 0.5};
        return <InputGroup.Append onClick={onClick} style={appendStyle}>
            <InputGroup.Text style={textStyle}>{this.props.clearButtonElement}</InputGroup.Text>
        </InputGroup.Append>;
      }
  }
  ...
  renderOverlay() {
    return <>
        ...
        <Overlay
          ...
          delayHide={200}>
        <Popover id={`date-picker-popover-${this.props.instanceCount}`} className="date-picker-popover">
          <Popover.Title>{this.renderCalendarHeader()}</Popover.Title>
          <Popover.Content>
            <DatePickerCalendar
